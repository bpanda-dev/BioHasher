...
SequenceDataGenerator::SequenceDataGenerator(SequenceRecordsWithMetadata *sequenceRecordsWithMetadata){
	assert(sequenceRecordsWithMetadata != nullptr);
	assert((sequenceRecordsWithMetadata->KeyCount > 0) && "Key count should be greater than 0");
	...
	RandGen.seed(sequenceRecordsWithMetadata->DatagenSeed);
...
void SequenceDataGenerator::generateWithDistribution(SequenceRecordsWithMetadata *sequenceRecordsWithMetadata) {

	std::uniform_real_distribution<> BaseDist(0,1);
	...
	for (uint32_t rec_idx = 0; rec_idx < sequenceRecordsWithMetadata->KeyCount; rec_idx++) {
		...
		const uint32_t keyBytes = ((sequenceRecordsWithMetadata->OriginalSequenceLength * two_bit_per_base ) + 7) / 8;
		...
		for (uint32_t pos = 0; pos < sequenceRecordsWithMetadata->OriginalSequenceLength; pos++) {
			...
			double randValue = BaseDist(RandGen);

			uint8_t base;

			if (randValue < sequenceRecordsWithMetadata->A_percentage) {
				base = 0; // A
			} else if (randValue < (sequenceRecordsWithMetadata->A_percentage + sequenceRecordsWithMetadata->C_percentage)) {
				base = 1; // C
			} else if (randValue < (sequenceRecordsWithMetadata->A_percentage + sequenceRecordsWithMetadata->C_percentage +
...
void SequenceDataGenerator::FillRecords(SequenceRecordsWithMetadata *sequenceRecordsWithMetadata) {
	// Generate sequences with specified nucleotide distribution
	generateWithDistribution(sequenceRecordsWithMetadata);
}

//------------------------------------------------------------------------------------------

SequenceDataMutator::SequenceDataMutator(SequenceRecordsWithMetadata *sequenceRecordsWithMetadata) {
	assert(sequenceRecordsWithMetadata != nullptr);
	assert(sequenceRecordsWithMetadata->Records.size() > 0);
	assert((sequenceRecordsWithMetadata->snpRate >= 0.0) && (sequenceRecordsWithMetadata->snpRate <= 1.0) && "SNP rate should be between 0 and 1");
	assert((sequenceRecordsWithMetadata->smallIndelRate >= 0.0) && (sequenceRecordsWithMetadata->smallIndelRate <= 1.0) && "Small indel rate should be between 0 and 1");
	// Initialise seed and reserve memory.
	RandGen.seed(sequenceRecordsWithMetadata->DataMutateSeed);
}
...
bool SequenceDataMutator::simulateSNP(SequenceRecordUnit &record, const uint32_t pos, std::mt19937 &randGen) {

	// Get current base
	// uint8_t currentBase = GetBaseAtPosition(record.SeqTwoBitMut, pos);	// Disabled: working on ASCII sequence for mutation simulation
	...
	uint8_t newBaseid = dist(randGen) & 0x03;
	...
	// SetBaseAtPosition(record.SeqTwoBitMut, pos, newBase);	// Disabled: working on ASCII sequence for mutation simulation

	// Update edit distance
	// record.HammingDistance += 1;
	return true;
}

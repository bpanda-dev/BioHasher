...
def main():
    ...
    parser.add_argument("--collision", type=str, help="Path to the raw Collision Curve test output CSV file.")
    parser.add_argument("--show", action="store_true", help="Launch browser with the generated dashboard immediately.")
    
    args = parser.parse_args()
    
    if not args.ann and not args.collision:
        print("Error: You must provide at least one of --ann or --collision.")
        parser.print_help()
        sys.exit(1)
    ...
    if args.collision:
        if not os.path.exists(args.collision):
            print(f"Error: Collision file '{args.collision}' not found.")
            sys.exit(1)
            
        print(f"=== Processing Collision Curve data: {args.collision} ===")
        cmd = [sys.executable, os.path.join(script_dir, "plot_collisioncurve.py"), args.collision]
        try:
        ...
        similarity_name = get_similarity_name(args.collision)
        csv_dir = os.path.dirname(os.path.abspath(args.collision))
        processed_file = os.path.join(csv_dir, similarity_name, f"{similarity_name}_processed.csv")
    ...
    if args.ann:
        ...
        processed_file = os.path.join(csv_dir, similarity_name, f"{similarity_name}_processed-ann.csv")

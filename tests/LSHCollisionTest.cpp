...
static void LSHCollisionTestInnerInnerWorker(const HashInfo * hinfo, uint32_t N_hash, HashFn hash, seed_t HashSeed, SequenceRecordsWithMetadataStruct &sequenceRecordsforTest,const std::vector<UnionBitVectorsStruct> *precomputedVectors, double *AverageCollision,int start, int end){
  ...
  for (int rec_idx = start; rec_idx < end; rec_idx++) {
    ...
    if (useUniverseOpt && precomputedVectors != nullptr) {
      ...
      const UnionBitVectorsStruct &unionBitVectors =
          (*precomputedVectors)[rec_idx];
      ...
      for (uint32_t hash_idx = 0; hash_idx < N_hash; hash_idx++) {
        ...
        hash(vecA_ptr, vecA_len, HashSeed + 7 * hash_idx + 17, &hash_val_org);
        hash(vecB_ptr, vecB_len, HashSeed + 7 * hash_idx + 17, &hash_val_mut);
      ...
      for (uint32_t hash_idx = 0; hash_idx < N_hash; hash_idx++) {
        ...
        hash(ptrOrg, lenOrg, HashSeed + 7 * hash_idx + 17, &hash_val_org);
        hash(ptrMut, lenMut, HashSeed + 7 * hash_idx + 17, &hash_val_mut);
    ...
    double avg_collision =
        static_cast<double>(collision_count) / static_cast<double>(N_hash);
    AverageCollision[rec_idx] = avg_collision;
  }
}

template <typename hashtype>
static bool LSHCollisionTestInnerInnerParallel(
    const HashInfo *hinfo, uint32_t N_seq, uint32_t N_hash, HashFn hash,
    seed_t HashSeed, common_params_struct &common_params,
    sim_bins_struct &sim_bins, SeedGenerator &seedGen,
  ...
  sequenceRecordsforTest.isBasesDrawnFromUniformDist =
      common_params.isBasesDrawnFromUniformDist;
  sequenceRecordsforTest.DatagenSeed = seedGen.nextSeed();
  ...
  // For each of the sequence pair, draw a random mutation parameter from the
  // bin statistics. Then store it in the mutation record. and mutate it. seed_t
  // bin_sampling_seed = seedGen.nextSeed(); seed_t bin_params_sampling_seed =
  // seedGen.nextSeed();
  ...
  for (uint32_t idx = 0; idx < N_seq; idx++) {
    ...
    sequenceRecordsforTest.Records[idx].foundationalParameter =
        sampled_error_param;
  }
  ...
  if (useUniverseOpt) {
    ...
    for (uint32_t i = 0; i < N_seq; i++) {
      precomputedVectors[i] = CreateUnionBitVectors(
          ...
          sequenceRecordsforTest.Records[i].SeqASCIIMut, tokenLen);
    }
  ...
  const std::vector<UnionBitVectorsStruct> *vecPtr =
      useUniverseOpt ? &precomputedVectors : nullptr;
  ...
  if (g_NCPU == 1) {
    ...
    LSHCollisionTestInnerInnerWorker<hashtype>(
        hinfo, N_hash, hash, HashSeed, sequenceRecordsforTest, vecPtr,
    ...
    for (uint32_t i = 0; i < g_NCPU; i++) {
      ...
      threads[i] =
          std::thread(LSHCollisionTestInnerInnerWorker<hashtype>, hinfo, N_hash,
                      hash, HashSeed, std::ref(sequenceRecordsforTest), vecPtr,
                      AverageCollision.data(), start, end);
    }
...
static sim_bins_struct
LSHCollisionTestInnerAgg(uint32_t N_agg, common_params_struct &common_params,
                         SeedGenerator &seedGen, std::ofstream &out_file) {
  ...
  sequenceRecordsForAgg.isBasesDrawnFromUniformDist =
      common_params.isBasesDrawnFromUniformDist;
  ...
  if (g_mutation_model == MUTATION_MODEL_SIMPLE_SNP_ONLY) {
    ...
    printf("Using geometric mutator model for data mutation in aggregation "
           "phase.\n");
  ...
  for (size_t i = 0; i < N_agg; i++) {
    ...
    uint32_t bin_index =
        static_cast<uint32_t>((sim_value - sequenceRecordsForAgg.binstart) /
                              sequenceRecordsForAgg.binsize);
    ...
    if (bin_fill_count == g_bincount_full) {
      continue; // To avoid excessive memory usage, we limit to x entries per
                // bin.
    }

    sim_bins_agg.bin_error_parameters[bin_index][bin_fill_count] =
        rand_error_param[i];
  ...
  for (size_t bin_idx = 0; bin_idx < sequenceRecordsForAgg.bincount;
    ...
    if (!params.empty()) {
      ...
      for (uint32_t param_idx = 0;
           param_idx < sim_bins_agg.bin_fill_count[bin_idx]; param_idx++) {
  ...
  for (size_t bin_idx = 0; bin_idx < sequenceRecordsForAgg.bincount;
    ...
    if (current_fill < g_bincount_full && current_fill > 0) {
      ...
      for (uint32_t fill_idx = current_fill; fill_idx < g_bincount_full;
           fill_idx++) {
...
template <typename hashtype>
static bool LSHCollisionTestInner(const HashInfo *hinfo, const uint32_t seqLen,
                                  flags_t flags, std::ofstream &out_file,
  ...
  out_file << ":2:" << "Hashname," << "SequenceLength," << "TokenLength,"
           ...
           << std::endl;
  out_file << ":3:" << hinfo->name << "," << seqLen << "," << tokenlength << ","
  ...
  // 	out_file << ":4:" << g_subseqHash1_subseq_len << "," << g_subseqHash1_d
  // << std::endl;
  // }
  ...
  common_params.isBasesDrawnFromUniformDist = true;
  common_params.distanceClass = setDistanceClassForHashInfo(hinfo->hash_flags);
  //--------------------------------------------//

  if (!REPORT(VERBOSE, flags)) {
    // printf("LSH Collision Test: Key Size = %3u bits (%2u bytes), Keys = %8u,
    // Hashes per Key = %4zu\n",
...
bool LSHCollisionTest(const HashInfo *hinfo, flags_t flags) {
  ...
  std::string filename =
      "../results/collisionResults_" + std::string(hinfo->name) + ".csv";
  ...
  if (hinfo->hash_flags & FLAG_HASH_HAMMING_SIMILARITY) {
    printf("Hash %s uses Hamming similarity. Setting mutation model to simple "
           "SNP only for testing.\n",
           hinfo->name);
  ...
  */
  ...
  for (const auto &toklen : tokenlengths) {
    ...
    for (const auto &seqLen : sequenceLengths) {
      ...
      uint32_t keybits =
          seqLen * 8; // Eg: Keybits = Sequence length times 8 (8 bits per base)

      // --- Sanity Checks for Test Configuration Only valid for tokenised
      // hashes ---	//
      if (hinfo->hasTokenisationProperty()) {
        // Condition 1: Skipping if token length is greater than sequence
        // length.
        if (toklen > seqLen) {
        ...
        const uint64_t MIN_TOKENS_FOR_MUTATION =
            20; // A more reasonable threshold.
        if (tokensInSequence < MIN_TOKENS_FOR_MUTATION) {
          printf("Skipping: Sequence with %llu tokens is too short (min is "
                 "%llu).\n",
                 (unsigned long long)tokensInSequence,
                 (unsigned long long)MIN_TOKENS_FOR_MUTATION);
          continue;
        }

        // Condition 3: Skipping if the token space is too small, which would
        // lead to high collision rates even for random data, making the LSH
        // property test less meaningful.
        if (toklen > 0) {
          uint64_t maxPossibleTokens =
              (toklen >= 32) ? UINT64_MAX : (1ULL << (2 * toklen));

          // This checks if the number of tokens in the sequence is a large
          // fraction of the total possible unique tokens. For example, skip if
          // tokensInSequence > 50% of maxPossibleTokens.
          if (maxPossibleTokens / 2 < tokensInSequence) {
            printf("Skipping: High token repetition expected. Tokens in "
                   "sequence (%llu) vs. max possible (%llu).\n",
                   (unsigned long long)tokensInSequence,
                   (unsigned long long)maxPossibleTokens);
            continue;
          }
        }
      ...
      baseSeed = seedGen.nextSeed();
      printf("\nTesting hash: %s with keybits: %u (sequence length: %u), token "
             "length: %d\n",

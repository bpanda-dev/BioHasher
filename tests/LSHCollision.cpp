...
static void LSHCollisionTestInnerInnerWorker(const HashInfo * hinfo, uint32_t N_hash, HashFn hash, seed_t HashSeed, common_params_struct &common_params, SequenceRecordsWithMetadataStruct &sequenceRecordsforTest, double *AverageCollision,int start, int end, uint32_t and_param, uint32_t or_param){
...
static bool LSHCollisionTestInnerInnerParallel(const HashInfo * hinfo, uint32_t N_seq, uint32_t N_hash, HashFn hash, seed_t HashSeed, common_params_struct &common_params, sim_bins_struct &sim_bins, SeedGenerator &seedGen, std::ofstream &out_file){
	...
	assert(g_start_B > 0 && g_start_R > 0 && g_MAX_B >= g_start_B && g_MAX_R >= g_start_R);
	...
	for(auto andor_param : AND_OR_params){
		...
		if (g_NCPU == 1) {	// Single threaded fallback
			...
			LSHCollisionTestInnerInnerWorker<hashtype>(hinfo, N_hash, hash, HashSeed, common_params, sequenceRecordsforTest, AverageCollision.data(), 0, N_seq, and_param, or_param);
			...
			printf("Sequential collision computation took %.3f seconds.\n", elapsed_seq.count());
				...
				for(uint32_t i = 0; i < g_NCPU; i++){
					...
					threads[i] = std::thread(LSHCollisionTestInnerInnerWorker<hashtype>, hinfo, N_hash, hash, HashSeed, std::ref(common_params), std::ref(sequenceRecordsforTest), AverageCollision.data(), start, end, and_param, or_param);
				}

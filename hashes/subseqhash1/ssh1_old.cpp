...
static void init_state(SubseqHashState& state, int blen, int p, seed_t seed){
	assert(p > 0 && "Parameter 'p' (or 'd') must be positive.");
	...
	for(int i = 0; i < blen; i++)
	{
		...
		for(int j = 0; j < 4; j++) {
            // If p < 4, we can't assign a unique value to each base.
            // Using modulo allows assignment to wrap around, preventing out-of-bounds access.
            // This ensures that state.sign[i][j] gets a valid value from the 'pos' vector.
			state.sign[i][j] = pos[j % p];
        }
